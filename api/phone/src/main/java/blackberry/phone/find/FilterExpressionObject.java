...
public class FilterExpressionObject extends Scriptable implements TestableScriptableObject {
    ...
    public boolean test( final Scriptable object ) {
        ...
        if( leftField instanceof String ) {
            ...
            if( fieldToTest == null && getOperator() != FilterExpressionConstructor.OPERATOR_NOT_EQUAL
                    && getOperator() != FilterExpressionConstructor.OPERATOR_EQUAL ) {
                return isNegateExpression();
            } else {
                return Tester.test( fieldToTest, rightField, getOperator(), optional ) ? !isNegateExpression()
                        : isNegateExpression();
            }
