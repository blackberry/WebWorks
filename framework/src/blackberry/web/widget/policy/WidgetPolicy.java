...
public class WidgetPolicy {
    ...
    /**
     * Apply widget security model to the requested URI to retrieve the corresponding
        ...
        try {
            ...
            if (requestURI.isAbsolute()) {
                ...
                if(isMatch(_localAccess, requestURI)){
                    ...
                    if (authString.equals("") && !schemeString.equals("file")){
                        return null;
                    } 
    ...
    private boolean isMatch(WidgetAccess access, URI toMatchURI) {
        ...
        if (allowSub 
                && !matchHost.toLowerCase().endsWith("." + refHost.toLowerCase()) 
                && !matchHost.equalsIgnoreCase(refHost)) {
            return false;
        }
        // 4. Compare port
                ...
                for (int i = 0 ; i < accessList.length ; i++) {
                    ...
                    if(currentAccess.isLocal()){
                        ...
                        if(_authorityCollection.containsKey(currentURI.getScheme() + "://" + currentURI.getAuthority())){
                            folderAccess = (WidgetWebFolderAccess)_authorityCollection.get(currentURI.getScheme() + "://" + currentURI.getAuthority());
                        }
                        else{                            
                        ...
                        folderAccess.addWidgetAccess(currentURI.getPath(), currentAccess);
